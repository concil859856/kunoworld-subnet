...
KUNO_VALIDATOR_API_KEY=... kuno-validator run --interval 4320 --netuid <netuid> \
...
1. **Attestation.** Fetches every active enclave from `/validator/v1/enclaves`, issues a
   ...
   from the same keys the enclave registered. Nothing is delegated to a central service.
