...
| Attestation | a miner running different code or fake hardware | Intel TDX quote (DCAP, via dcap-qvl) plus NVIDIA GPU evidence (NRAS or `nvattest`), bound to a fresh validator nonce and to the enclave's own keys, checked against an owner-signed manifest of measurements. The gateway and every validator build the same policy (`KUNO_ATTESTATION=production` refuses to start without both verifiers). Clients check again for themselves before sealing a job: the gateway relays the Intel collateral and NVIDIA attestation tokens its check used (`endorsements`, [PROTOCOL.md](PROTOCOL.md#endorsements-what-clients-check-tdx-evidence-with)), and both SDKs and the studio verify them under Intel's pinned root and NVIDIA's pinned attestation intermediate, and can pin the owner's key rather than trust the gateway's copy of the manifest — *tested against Intel's sample quotes, NVIDIA's real signing certificates and simulated NVIDIA tokens; not yet run against a live TDX + NVIDIA CC machine*. **Open-tier miners (`tee: "open"`) attest nothing**: they are admitted only where the owner-signed manifest enables the open tier, serve standard jobs only, never private ones, and rely on step audits, collateral and admission probes instead ([PRIVACY_MODES.md](PRIVACY_MODES.md)) |
...
| Licence territory | MiniMax H3 running where its licence forbids it (the US, EU, UK, South Korea) | an enclave offering H3 times signed pings to KunoWorld's landmark servers from inside its confidential VM; physics caps how far a fast round trip can reach, so a fast enough answer from a landmark far from every excluded territory proves the machine is outside them, whatever its IP address says ([PROTOCOL.md](PROTOCOL.md#location-proofs)) — *no landmark is deployed yet; the gateway otherwise falls back on the country of the worker's connection* |
...
- **What the model itself does.** Attestation proves which code ran, not that the model is
  ...
  The limits today:
  - Both worker images bake in Qwen3Guard-Gen-0.6B, Freepik's detector and CLIP, each pinned
    by revision and SHA-256 (`image/CVM.md`). All three load and answer on CPU through the worker's
    own code (`kuno-safety-check`), on benign synthetic inputs only. None has been evaluated for
    accuracy, run inside a CVM, or run beside a GPU workload.
...
- running attestation end to end on a real TDX host with NVIDIA GPUs in CC mode: nothing
  below has produced or verified a live quote or live GPU evidence, and no real NRAS token has been checked by the
  clients' endorsement verifiers (their token logic is proven on local tokens shaped like NRAS's);
...
- the safety pipeline, with fake classifiers, including the shared content policy and the
  output check wired into the worker. The worker images' pinned prompt and frame classifiers
  also load and score on CPU through the worker's code (`kuno-safety-check`), on benign inputs only;
